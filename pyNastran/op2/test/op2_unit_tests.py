...
class TestOP2(Tester):
    ...
    def test_nx_composite_solids_nonlinear(self):
        """
        checks nx/composite_solids/nonlinear_sol402.dat, which tests
        centroidal CHEXA composite stress in case of a nonlinear simulation
        """
        log = get_logger(level='info')
        bdf_filename = os.path.join(MODEL_PATH, 'nx', 'composite_solids', 'nonlinear_sol402.dat')
        op2_filename = os.path.join(MODEL_PATH, 'nx', 'composite_solids', 'nonlinear_sol402.op2')
        # bdf_filename = os.path.join(MODEL_PATH, 'nx', 'composite_solids', 'composite_solids_nonlinear_test_sim1-copy_of_solution_1.dat')
        # op2_filename = os.path.join(MODEL_PATH, 'nx', 'composite_solids', 'composite_solids_nonlinear_test_sim1-copy_of_solution_1.op2')
        op2 = OP2()  #
        bdf = BDF()
        # op2.read_op2(op2_filename=f"{common_folder}/ring_cpv_sim.op2")
        # bdf = help_functions.read_bdf(common_folder, "ring_cpv_sim.dat")
